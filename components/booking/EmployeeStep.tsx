...
type EmployeeStepProps = {
  ...
  selectedEmployeePreference: "any" | string | null;
...
export default function EmployeeStep({
  ...
  if (!selectedServiceId) {
    return (
      <EmptyState
        title={translations.booking.selectServiceFirst}
        description={
  ...
  const availableEmployees = employees.filter(
    (employee) =>
      employee.isActive &&
      employee.serviceIds.includes(selectedServiceId)
  );
  ...
  if (availableEmployees.length === 0) {
    return (
      <EmptyState
        ...
        description={
          translations.common.noEmployeesForService
        }
        icon={Users}
        locale={locale}
      />
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        title={translations.booking.selectEmployee}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <button
          type="button"
          onClick={() =>
            onSelectEmployeePreference("any")
          }
          ...
          <span className="flex items-center gap-4">
            ...
            <span className="flex-1">
              <span className="font-display mb-1 block text-lg font-semibold text-[var(--brand-text)]">
                {t(translations.booking.any, locale)}
              </span>
        ...
        {availableEmployees.map((employee) => (
          <EmployeeCard
            ...
            mode="selectable"
            isSelected={
              selectedEmployeePreference === employee.id
            }
            onSelect={onSelectEmployeePreference}
          />
        ))}
      </div>
    </div>
